...
FULL COMBINATION LOGIC:
  ...
  ENTER when ALL true:
    ...
    - Hawkes Score > 1 (TREND) or > 2 (SIDEWAYS)

    - Hawkes Intensity > 2 (TREND) or > 2.5 (SIDEWAYS)
...
# =========================================================================
# COMBO SIGNAL: Full HMM + Hawkes Combined
# =========================================================================
...
def _combo_signal(hmm_regime, hmm_confidence, score, intensity, events, qqq_trend_ok, closes):
    ...
    EXIT when ANY true:
      ...
      - Hawkes Intensity > 8 (overheated)
    """
    global _tick, _entry_tick

    # ===== EXIT CONDITIONS (priority order) =====

    # 1. CRASH: Always exit on breakdown
    if events.get('breakdown', 0):
        return "exit"

    # 2. HIGH VOLATILITY REGIME: Exit when HMM says HIGH_VOL
    if hmm_regime == "HIGH_VOLATILITY":
        return "exit"

    # 3. Market stress (SPY or QQQ dropping fast)
    spy_stress = _market_stress_rate("SPY")
    qqq_stress = _market_stress_rate("QQQ")
    if spy_stress > MARKET_STRESS_THRESHOLD or qqq_stress > MARKET_STRESS_THRESHOLD:
        return "exit"

    # 4. Hawkes Intensity > 8 (overheated)
    if intensity > EXIT_INTENSITY_OVERHEAT:
        return "exit"

    # 5. HMM Confidence < 0.20 (lost confidence)
    if hmm_confidence < EXIT_HMM_CONFIDENCE:
        return "exit"

    # 6. Hawkes Score = 0 (no signal)
    if score <= EXIT_HAWKES_SCORE:
        return "exit"
    ...
    if hmm_regime == "TREND":
        ...
        # SIDEWAYS: Stronger Hawkes signals required
        if hmm_confidence <= 0.50:
            return "exit"
        if score <= 2.0:
            return "exit"
